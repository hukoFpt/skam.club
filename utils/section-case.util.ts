...
export const getCasesBySection = (sectionId: string): Case[] => {
  ...
  return casesData.filter((caseItem) => caseIds.includes(caseItem.id)) as Case[];
};
...
export const getCasesByMultipleSections = (sectionIds: string[]): Case[] => {
  const allCaseIds = sectionIds.flatMap((sectionId) => SECTION_CASE_MAPPING[sectionId] || []);

  return casesData.filter((caseItem) => allCaseIds.includes(caseItem.id)) as Case[];
};
